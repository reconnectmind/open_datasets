...
Каноническая таблица: [`public/registry.csv`](./public/registry.csv). Одна строка соответствует одному датасету или независимо скачиваемой части датасета. Основные сведения, счётчики, диапазоны длительности raw fNIRS-записей и инвентарь сопутствующих признаков объединены в этом файле; репозитории и каталоги сами по себе в таблицу не включены.

## Просмотр

[`public/index.html`](./public/index.html) — самодостаточный SPA-просмотрщик поверх `registry.csv` (поиск, фильтры по источнику/лицензии/модальности/статусу, карточки-статистика, детальная панель с раскодированными признаками). `index.html` и `registry.csv` лежат рядом в `public/`, которая целиком публикуется на GitHub Pages. Развернуто yf reconnectmind.github.com/open_datasets&. 
...
| `fnirs_duration_min_s`, `fnirs_duration_max_s` | Минимальная и максимальная длительность одной independently analyzable raw fNIRS-записи в секундах |
| `duration_basis` | Как получен диапазон: sample count/rate, временной вектор, заголовок файла или тайминг протокола |
| `duration_status` | `verified`, если проверены сами raw-метаданные; `partial`, если доступен только протокол или неполная граница; `unavailable`, если значение нельзя обосновать |
...
Для всех 48 строк теперь задан проверяемый результат подсчёта: 35 счётчиков подтверждены по file tree/manifest, 11 основаны на подтверждённой, но не полностью перечисленной структуре, а 2 имеют предупреждение о metadata. Ноль у `ds007477` означает отсутствие валидных raw-записей, а не неизвестное значение.

### Длительность fNIRS-записи

Диапазон считается по той же independently analyzable raw-единице, что и `n_recordings`: по отдельному SNIRF/NIRx/WFDB-файлу или подтверждённому participant/run-сегменту. Записи разных участников и runs не складываются; derivatives и отдельные trials внутри непрерывного raw-файла не считаются самостоятельной длительностью.

- `verified` — min/max вычислены из числа samples и частоты, временного вектора либо raw-заголовка;
- `partial` — известен опубликованный тайминг протокола, приблизительный диапазон сессии или только одна граница;
- `unavailable` — raw fNIRS отсутствует/недоступен и полного тайминга протокола недостаточно.

Из 48 строк 34 имеют проверенный sample-level диапазон, 11 — частичный protocol-level диапазон, 3 — недоступны. Значения хранятся в секундах без преждевременного округления; интерфейс показывает их в удобном человекочитаемом виде.
...
| --- | --- | --- | --- | ---: | --- | ---: | --- | --- | --- |
| `24032265` | Figshare v1 | Chinese colour-word Stroop: синхронные fNIRS, EEG, EOG и поведение | Здоровые взрослые / 21 | 21 ✓ | fNIRS + EEG + EOG + behavior / TDMS, MAT, CNT, CSV/XLS | 2.05 GB | CC0 | [прямой ZIP](https://ndownloader.figshare.com/files/42141585) | verified-core; 21 TDMS подтверждены по вложенному ZIP manifest |
